...
import { Identity } from "../attestation/ASN1Schemas";
import {
  createAttestationRquestAndSecret,
  parseAttestation,
} from "../attestation/AttesationUtils";
import { str2ab } from "../common/Utils";

xtest("createAttestationRquestAndSecret should work", async () => {
  const result = await createAttestationRquestAndSecret(
    "mail",
    "test@mail.com",
...
test("parseAttestation should work", () => {
  ...
  const notAfter = attestation.signedInfo.validity.value.notAfter.generalizedTime.getTime();
  const notBefore = attestation.signedInfo.validity.value.notBefore.generalizedTime.getTime();
