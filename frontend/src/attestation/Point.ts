...
export class Point {
  ...
  constructor(
    public x: bigint,
    public y: bigint,
    private useCurve: { [index: string]: bigint } = CURVE_SECP256k1
  ) {}
  ...
  double(): Point {
    ...
    const Y1 = this.y;
    const lam = mod(
      3n * X1 ** 2n * invert(2n * Y1, this.useCurve.P),
      this.useCurve.P
    );
  ...
  add(other: Point): Point {
    ...
    const lam = mod(
      (Y2 - Y1) * invert(X2 - X1, this.useCurve.P),
      this.useCurve.P
    );
  ...
  static decodeFromHex(
    hex: string,
    useCurve: { [index: string]: bigint } = CURVE_SECP256k1
  ) {
    if (hex.length != 130) {
